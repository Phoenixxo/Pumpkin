...
impl DatapackManager {
    ...
    pub fn get_pack_info(server: &Server, name_or_id: &str) -> Option<DatapackInfo> {
        ...
        let repository = server
            ...
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let pack = repository.get_pack(&resolved_name)?;
        let is_enabled = repository.selected_ids().contains(&resolved_name);
        let (description, pack_format) = pack.metadata.as_ref().map_or_else(
            || {
                (
                    if resolved_name == "vanilla" {
                        "The default data pack".to_string()
                    } else {
                        String::new()
                    },
                    pumpkin_datapack::pack::format::PackFormat::CURRENT.major,
                )
            },
            |metadata| {
                let description = metadata
                    ...
                    .description
                    .as_str()
                    .map_or_else(|| metadata.pack.description.to_string(), str::to_string);
                (description, metadata.pack.pack_format)
            },
        );
    ...
    pub fn execute_function_from_console(
        ...
        server.execute_datapack_function(name)
    }
}
